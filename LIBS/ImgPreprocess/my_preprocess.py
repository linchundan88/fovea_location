...
get_fundus_border: only used in Fovea_center/DR0_4_inconsistent/crop_images
...
def detect_xyr(img_source):
    ...
    #if param=1 0da9573e-b4dd-4852-af91-ccd549c032fe.13 error
    circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, minDist=450, param1=160, param2=32,
...
def do_preprocess_rop(img_source, crop_size, img_file_dest=None, add_black_pixel_ratio=0):
    if isinstance(img_source, str):
        try:
            img1 = cv2.imread(img_source)
        except:
    ...
    if add_black_pixel_ratio > 0:
        img2 = add_black_margin(img2, add_black_pixel_ratio=add_black_pixel_ratio)
