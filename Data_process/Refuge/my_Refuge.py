...
dir_original = '/media/ubuntu/data2/Fover_center/Refuge/original'
dir_preprocess = '/media/ubuntu/data2/Fover_center/Refuge/preprocess512'
do_preprocess = True
dir_tmp = '/tmp2/Refuge'
preprocess_image_size = 512

filename_csv = os.path.join(os.path.abspath('.'), 'refuge.csv')
...
with open(filename_csv, 'w', newline='') as csvfile:
    ...
    for file_type in ['Fovea_location_training.xlsx', 'Fovea_locations_validation.xlsx']:
        filename_excel = os.path.join(os.path.abspath('.'), file_type)
        df = pd.read_excel(filename_excel)
        for _, row in df.iterrows():
            print(row['ImgName'], row['Fovea_X'], row['Fovea_Y'])
            image_file_original = os.path.join(dir_original, row['ImgName'])
            img1 = cv2.imread(image_file_original)
            (height, width) = img1.shape[0:2]

            x, y = row['Fovea_X'], row['Fovea_Y']
